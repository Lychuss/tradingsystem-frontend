...
export default function Header() {
  ...
  return (
    <>
      ...
      <Text />
      <CgProfile onClick={() => setShow(!show)} className="inline-block cursor-pointer rounded-sm text-white w-13 h-13 p-2" />
      {show && (        
        <div className="absolute z-50 bg-gray-800 w-40 h-40 right-15 rounded-lg">
            <ul>
                <li className="border-b border-black p-2 hover:rounded-md hover:bg-gray-700 cursor-pointer transition">Profile</li>
                ...
                <li className="border-black border-b p-2 hover:rounded-md hover:bg-gray-700 cursor-pointer transition">Settings</li>
            </ul>
        </div>
    )}
    </>
